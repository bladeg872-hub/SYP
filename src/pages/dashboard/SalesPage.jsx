...
function SalesPage() {
  ...
  const handleOpenEdit = (entry) => {
    ...
    setEditFormData({
      amount: entry.amount,
      buyer_pan_vat: entry.buyer_pan_vat,
  ...
  const handleEditEntry = async (event) => {
    ...
    if (!validatePanVatNumber(editFormData.buyer_pan_vat)) {
      setError(t('salesErrPanVat'))
      return
    }
    ...
    try {
      const response = await fetch(`${SALES_ENDPOINTS.listCreate}${editingEntry.id}/`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteEntry = async (entryId) => {
    ...
    try {
      const response = await fetch(`${SALES_ENDPOINTS.listCreate}${entryId}/`, {
        method: 'DELETE',
      ...
      {editingEntry ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-lg">
            <h3 className="text-lg font-semibold text-gray-900">Edit Sales Entry</h3>
            <form className="mt-4 space-y-4" onSubmit={handleEditEntry}>
              ...
              <FormInput
                label="Buyer PAN/VAT"
                value={editFormData.buyer_pan_vat}
                onChange={(e) => setEditFormData({ ...editFormData, buyer_pan_vat: e.target.value })}
                required
              />
              <DatePickerInput
                label="Date (BS)"
